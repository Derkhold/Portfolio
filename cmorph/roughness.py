...
def dfa_alpha(path: np.ndarray, orders: Tuple[int,...]=(1,), scales: np.ndarray | None = None) -> float:
    ...
    L = len(x)
    if scales is None:
        scales = np.unique(np.round(np.linspace(4, max(4, L//2), num=min(8, max(2, L//2)))).astype(int))
